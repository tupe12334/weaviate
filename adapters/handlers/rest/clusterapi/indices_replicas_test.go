...
// buildBinaryDigests encodes a slice of RepairResponse records into the 24-byte
// binary wire format used by postCompareDigests.
func buildBinaryDigests(t *testing.T, records []types.RepairResponse) []byte {
	...
	for i, r := range records {
		...
		require.NoError(t, err)
		copy(buf[i*replica.DigestObjectsInRangeRecordLength:], id[:])
...
func TestPostCompareDigests(t *testing.T) {
	...
	t.Run("HappyPath", func(t *testing.T) {
		...
		require.NoError(t, err)
		require.Equal(t, len(staleDigests)*replica.DigestObjectsInRangeRecordLength, len(respBody))

		// Decode and verify each returned stale record.
		for i, want := range staleDigests {
			off := i * replica.DigestObjectsInRangeRecordLength
			rec := respBody[off : off+replica.DigestObjectsInRangeRecordLength]
			gotID, err := uuid.FromBytes(rec[:16])
			require.NoError(t, err)
			assert.Equal(t, want.ID, gotID.String())
			assert.Equal(t, uint64(want.UpdateTime), binary.BigEndian.Uint64(rec[16:]))
		}
	})
	...
	t.Run("InvalidPayloadLength", func(t *testing.T) {
		t.Parallel()
		// A payload of 10 bytes is not a multiple of DigestObjectsInRangeRecordLength (24).
	...
	t.Run("PayloadTooLarge", func(t *testing.T) {
		...
		// Send one byte beyond the max (10_000 records × 24 bytes + 24 extra bytes).
		oversized := make([]byte, 10_001*replica.DigestObjectsInRangeRecordLength)
		req, err := http.NewRequestWithContext(context.Background(), http.MethodPost, url,
